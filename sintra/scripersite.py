...
class URL:
    ...
    @property
    def value(self):
        """str: Return the value of the URL. """
        if self._val == '#':
            return '#'
        else:
            return self._val
...
class WebPage(bs4.BeautifulSoup):
    ...
    def __init__(self, url: URL, content: str, **kwargs):
        ...
        Args:
            url (:obj:`URL`): URL of this web page.
            content (str): Content
        """
...
class Navigation(th.Thread):
    ...
    def __init__(self, *args, levn=0, **kwargs):
        ...
        #: int: Define the level of the navigation on web.
        self._levn = levn if levn is not None\
            else 0

    @property
    def url(self):
        """:obj:`URL`: Return the current URL of this nativation."""
        return self._curl
    ...
    @property
    def level(self):
        """int: Return the value of navigation level. """
        return self._levn
    ...
    def level(self, value):
        ...
        Raise:
            ValueError: The navigation level cannot be a 
                nigative integer.
        """
        if value < 0:
            raise ValueError(
                "The navigation level cannot be a nigative integer."
                )
        self._levn = value

    @property
    def webpages(self):
        """dict: Return the dictionnary of web page indexed by URLs. """
        return self._wps

    def __call__(self, url: URL):
        """Navigation function

        Redifinied function to call the navigation function
        when it called.
        """
    ...
    def _nav(self, url: URL, lev: int=None):
        """
        Function that is used to execute a navigation 
        to a website using a URL. 

        :param: url [URL]  The URL that index a website that want 
        to retreive.
        :return:
            [WebSite]
            We return an instance of WebSite found at the URL passed
            in argument.
        """
        try:
            ...
            if response.status_code == 200:
                ...
                print(SUCC + "STATUS CODE - 200 | {} is scrapped."\
                    .format(url))
                ...
                print(ERRO + "STATUS CODE RETURNED - {} for {}"\
...
if __name__ == '__main__':
    ...
    #    print(INFO + "{} WEB PAGE(S) SCRAPPED !"\
    #       .format(len(w.sub_web_pages)))
